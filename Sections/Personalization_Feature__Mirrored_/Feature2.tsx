    ...
    return (
        <section className="w-full bg-[#4F46E5] overflow-hidden min-h-[900px] flex">
            {/* Stacks vertically on mobile/tablet (< lg), horizontal on desktop (lg+). 
                Using flex-row-reverse on desktop to put the Image (2nd child) on the Left.
            */}
            <div className="flex flex-col lg:flex-row-reverse w-full">

                {/* Left Content Side (Now on Right due to row-reverse) */}
                {/* items-center on mobile (center text), items-start on desktop (align left side of col, towards image) */}
                <div className="w-full lg:w-1/2 p-8 md:p-16 flex flex-col justify-center items-center lg:items-start relative z-10">

                    {/* Content Block */}
                    {/* Centered on mobile, Left-aligned on Desktop. Swapped pr-12 to pl-12 for spacing from image. */}
                    <div className="w-full max-w-xl text-center lg:text-left lg:pl-12">
                ...
                {/* Right Visual Side (Now on Left due to row-reverse) - Darker Blue Background */}
